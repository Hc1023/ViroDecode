...
# python3 run_virodecode_docker.py -i sisih/viraldecode:1 -1 fq1 -2 fq2
# Required Arguments
...
parser.add_argument("-a", "--align", type=str, default="1", required=False,
                    help="Alignment tool  1: BOWTIE2 2: BWA MEM")
...
parser.add_argument("-p", "--amplicon", type=str, default="0", required=False,
                    help="Set 1 if amplicon sequencing")
parser.add_argument("-A", "--assembly", type=str, default="0", required=False,
                    help="Set 1 if needing assembly results (fasta)")
...
dir_bind = f"-v {r1Dir}:{r1Dir} -v {r2Dir}:{r2Dir} -v {OutDir}:{OutDir}"
if bedpe != "0":
    dir_bind += f"-v {os.path.abspath(bedpe)}:{os.path.abspath(bedpe)}"
...
if args.ref == "":
    ...
    os.system(
        f"sudo docker run {dir_bind}\
    {image} bash /workfolder/main.sh \
    ...
    os.system(
        f"sudo docker run {dir_bind} -v {ref}:{ref} \
    {image} bash /workfolder/main.sh \
    {InDir} {OutDir} {fq1} {fq2} {ref} {thread} \
    {align} {coverage} {gff} {genome_version} {amplicon} {bedpe} {assembly} \
    {min_reads2} {min_var_freq} {strand_filter} {clean}"
